...
3. **Painel Clínico de Atendimento & Parâmetros Laser**:
   ...
   - **Cruzamento de áreas** (`engines/catalogo-areas.js` + `engines/cross-sell.js`): mapa de **7 serviços**, **10 regiões** e **40 áreas** corporais. Compara o que a cliente já trata com o mapa completo e devolve a área vizinha que falta (faixa de barba sem o mento, meia perna sem a coxa), a região quase fechada, e o outro serviço aplicável na área que ela já faz (clareamento na axila que ela depila). Áreas exclusivas de um gênero são filtradas pelo `sexo` do cadastro (`buscardadoscliente`, uma requisição por cliente, cacheada 30 dias) ou, sem custo, pela dedução a partir das áreas que a pessoa já trata.
